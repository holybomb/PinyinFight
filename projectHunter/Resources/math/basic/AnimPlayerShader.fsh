...
// shui cao wave
uniform int	  enableShuiDouDong;
uniform float u_ShuiDouDongTime;
vec4 getShuiDouDong(vec4 rgbOrigin, vec2 texCoord)
{
	...
	vec4 mixin = texture2D(u_texture, texCoord);
 	...
 	vec4 ret = mixin * t + diffuse * (1.0 - t);
/*
	vec2 resolution;
	resolution.x = 1.0;
	resolution.y = 1.0;
	...
	vec2 uv = texCoord.xy/resolution.xy+(cPos/cLength)*cos(cLength*12.0-u_ShuiDouDongTime*4.0)*0.1;
...
void main(void) 
{
	...
	if(enableShuiDouDong == 1)
	{
		vec4 ret = getShuiDouDong(result, texCoord);
		result = ret;
	}
