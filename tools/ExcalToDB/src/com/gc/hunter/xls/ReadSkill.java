...
public class ReadSkill {
    ...
    public ReadSkill(boolean isUseBase64) throws Exception {
        isBase64 = isUseBase64;
        users = new IniEditor();
        users.load(ConfigName);
        String XlsFileName = users.get("skill", "sheetFiles");
    ...
    public void parse(Workbook workbook) throws Exception {
        ...
        String binFileName = users.get("skill", "binFiles");
        //生成的json文件名
        ...
        for (int page = 0; page < allSheet.length; page++) {
            ...
            try {
                for (int row = 0; row < curSheet.getRows(); row++) {
                    if (curSheet.getCell(0, row).getContents().toString().trim().equals("") && row > 2) {
                        break;
                    }
                    rowMax++;
                }
            ...
            for (int col = 0; col < curSheet.getRow(2).length; col++) {
                if (curSheet.getCell(col, 2).getContents().toString().trim().equals("")) {
                    continue;
                }
                //读取属性行（第一行）作为关键字
                indexTable.put("" + col, curSheet.getCell(col, 2).getContents().toString());
            }
        ...
        MainFrame.append("*******************************");
        MainFrame.append(" 技能表(sk.bin) " + MainFrame.getNowTime() + " 导出成功...");
        MainFrame.append("*******************************");
    }
